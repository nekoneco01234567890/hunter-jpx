...
def parse_sst(wb):
    """
    BIFF8 SST parser with CONTINUE-aware string decoding.

    Important:
      CONTINUE can occur in the middle of an SST string.
      When character data continues into a CONTINUE record,
      the first byte of that CONTINUE is the character-options
      byte controlling compressed/unicode representation.

    We therefore do NOT concatenate SST/CONTINUE blindly.
    """
    ...
    class Reader:
        ...
        def eof(self):
            return (
                self.si>=len(self.segments)
                or (
                    self.si==len(self.segments)-1
                    and self.pi>=len(self.segments[self.si])
                )
            )

        def position(self):
            return (self.si,self.pi)
        ...
        def read_char_bytes(self,n,unicode_mode):
            """
            Read n logical characters.

            If a CONTINUE boundary is crossed during character
            data, BIFF8 stores one option byte at the beginning
            of the CONTINUE segment. That option determines the
            encoding of the continuation character data.
            """
            ...
            while remaining>0:
                ...
                # Consume as much as possible from current segment.
                unit=2 if unicode_mode else 1
                ...
                if chars_here>0:
                    take=chars_here*unit
                    out += seg[self.pi:self.pi+take]
                    self.pi+=take
                ...
                # If character data continues into another
                # CONTINUE record, the next segment begins with
                # a character-options byte.
                if self.pi>=len(seg) and remaining>0:
                    ...
                    option=self.segments[self.si][self.pi]
                    self.pi+=1

                    unicode_mode=bool(option & 0x01)

            return bytes(out),unicode_mode
    ...
    for idx in range(unique):
        ...
        try:
            cch=r.read_u16()
            option=r.read_raw_byte()

            unicode_mode=bool(option & 0x01)
            has_rich=bool(option & 0x08)
            has_phonetic=bool(option & 0x04)

            rich_count=r.read_u16() if has_rich else 0
            phonetic_size=r.read_u32() if has_phonetic else 0

            if rich_count:
                r.read_raw(rich_count*4)

            raw,unicode_mode= r.read_char_bytes(
                cch,
                unicode_mode
            )

            # Decode according to the encoding actually used
            # for the character data.
            #
            # In ordinary files the whole string uses one mode.
            # For continuation cases the final mode may differ.
            #
            # The raw byte length determines the appropriate
            # decoder when continuation changed representation.
            if len(raw)==cch*2:
                text=raw.decode(
                    "utf-16le",
                    errors="replace"
    ...
    if len(strings)!=unique:
        raise ValueError(
            f"SST_COUNT_MISMATCH:{len(strings)}!={unique}"
        )
